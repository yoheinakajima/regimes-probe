...
| Level 2 query decomposition (v1) extracts clue **spans** (not single capitalized tokens), composes 3–6 targeted length-capped queries clause-by-clause, scores query quality and drops low-quality candidates; the bandit learns the query form (`tool × query_arm`) and the long full-question query is never the cold-start default. | `policy/query_decomposition.py` (`extract_clues`/`decompose`/`score_query`); `policy/query_policy.py` (`--enable-query-decomposition`); `tests/test_query_decomposition.py` (African/Mexican/monument/manga examples produce clue spans not `African One`/`Mexican`/`Name December 2023`; quality scoring drops single-generic queries; candidate list + drops surfaced in debug). |
| v0 decomposition reduced contamination but produced weak clue queries (single capitalized tokens); v1 uses clue spans + quality scoring. Scrape/fetch is to be evaluated only after query quality improves. | `docs/QUERY_POLICY.md` (v0→v1); `docs/METHODOLOGY_RISKS.md`. |
