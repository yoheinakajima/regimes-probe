...
prompt surfaces spam / benchmark-mirroring pages; extracting single capitalized
tokens (v0) produced weak/broad queries like ``African One`` / ``Mexican`` /
``Name December 2023``. v1 extracts meaningful **clue spans** (quoted phrases,
multiword entities, noun phrases around distinctive predicates, date+noun
phrases, rare terms), composes queries clause-by-clause, scores each candidate
for expected search quality, and DROPS low-quality candidates (single generic
tokens, pure-date, all-boilerplate).
...
DECOMPOSITION_ARMS: tuple[str, ...] = (
    ...
    "quoted_anchor_terms",
    "source_type_query",
    # iterative clue-resolution follow-up arms (see agent/clue_resolution.py):
    "candidate_entity_followup",
    "answer_shape_followup",
)
...
_STOPWORDS = {
    ...
    "later", "near", "every", "some", "most", "more", "very", "out", "up",
    "down", "over", "under", "per", "via", "within", "without",
}
_BOILER = {
    ...
    "want", "know", "figure", "former", "around", "approximately",
}
_GENERIC_ADJ = {
...
_COMMON_WORDS = {
    ...
    "family", "child", "woman", "history", "science", "language", "country",
    "region", "during", "between", "before", "around", "called", "named",
    "known", "famous", "popular", "modern", "classic", "century", "national",
    "general", "number", "company", "member", "group", "event", "title",
    "early", "decade", "period", "former", "winner", "founded", "another",
    "various", "certain", "several", "include", "including", "located",
...
def _trim_span(toks: list[str]) -> list[str]:
    """Drop leading/trailing boilerplate so a span starts/ends on content.
    Demonyms are not boilerplate, so 'Mexican restaurant' keeps 'Mexican' while
    'African One' trims to 'African' (then dropped as too short)."""
    i, j = 0, len(toks)
...
def _good_span(toks: list[str]) -> bool:
    if len(toks) < 2:
        return False
    if len([t for t in toks if _is_meaningful(t)]) < 2:
        return False
    return any(_is_noun_like(t) for t in toks)
...
class Clues:
    ...
    clause_clues: list[str] = field(default_factory=list)
...
def extract_clues(question: str) -> Clues:
    ...
    entities = _dedupe(_multiword_entities(question) + _single_entities(question))
...
def decompose(question: str, *, max_queries: int = 6, max_chars: int = MAX_QUERY_CHARS,
    ...
    if c.locations and (c.phrase_spans or c.entities):
        obj = next((s for s in c.phrase_spans if s.lower() not in
                    {loc.lower() for loc in c.locations}),
                   (c.phrase_spans[0] if c.phrase_spans else None))
